...
num_labels = len(set([k.split('_')[3] for k in h5_file.keys() if k.split('_')[0]=='label']))
print(num_cases, num_labels)
...
for _ in range(num_case_plots):
    ...
    plt.axis('off')
    plt.savefig('frames_sample/case{:04d}'.format(case_id)+'.jpg',bbox_inches='tight')
    plt.show()
    plt.close()
